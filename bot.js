...
async function generateAIResponse(userMessage, kb) {
  const context = `
You are "Humanoid", a helpful AI assistant for HumanoidCodeLab, a 3D humanoid robot programming platform.

App Information:
${JSON.stringify(kb.app_info, null, 2)}

FAQ:
${kb.faq.map(item => `Q: ${item.q}\nA: ${item.a}`).join('\n\n')}

Support Topics:
${Object.entries(kb.support_topics).map(([key, val]) => `${key}: ${val}`).join('\n')}

User Question: ${userMessage}

Provide a helpful, concise response. If the question is not related to HumanoidCodeLab, politely redirect the conversation.
  `.trim();
  ...
  try {
    ...
    return '❌ Sorry, I encountered an error. Please try again or contact support at humanoidcodelab@gmail.com';
  }
}
...
// Launch bot
bot.launch();
console.log('🤖 Humanoid bot is running...');
